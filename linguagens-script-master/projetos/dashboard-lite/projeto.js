...
function parserUOL(xml){
  ...
  $materias = $xml.find("item");
  
  $materias.each(function(){
    titulo = $(this).find('title').text();
    descricao = $(this).find('description').text();
    publicado = $(this).find('pubDate').text();
    dadosLista += criaElementoLista([titulo, descricao, publicado]);
  });
  document.getElementById("projeto_lista").innerHTML = dadosLista;
}
